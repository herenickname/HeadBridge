...
| Distribution | GitHub Releases; ad-hoc signed and not Apple-notarized |
...
The repository already includes two hardware-backed captures from the working
build. Future images should use the same macOS appearance and wallpaper, keep
the menu-bar icon visible where relevant, and avoid debug overlays.
...
| `assets/screenshots/settings-overview.png` | Included: both hardware providers connected in Settings | One app presents Bowers & Wilkins and Sony providers consistently. | HeadBridge Settings sidebar showing connected Px7 S3 and WH-1000XM3 headphones. |
| `assets/screenshots/sony-settings.png` | WH-1000XM3 settings with Noise Control and Sony sound controls visible | Sony MDR V1 support includes ambient sound, EQ, sound processing, and optional volume sync. | HeadBridge settings for Sony WH-1000XM3 with noise modes, EQ, and volume synchronization. |
| `assets/screenshots/battery-history.png` | One device's 24-hour or 7-day battery chart | Battery history is sampled locally while a headset is connected and remains available per device. | HeadBridge battery chart showing charge percentage over time for a connected headset. |

The two included images are the initial announcement set. Add the Sony and
battery-history close-ups when fresh release-build captures are available.

## Installation disclosure

Public releases are ad-hoc signed and are not signed or notarized by Apple. The optional installer removes quarantine from the installed `HeadBridge.app` bundle so it can open. That is a Gatekeeper bypass and should be described plainly wherever the installation command is shared. Users can inspect the installer, download manually, or build from source.
