...
1. matching its supported `SystemAudioDevice` names;
2. opening and owning exactly one vendor-control transport session;
3. automatically connecting only while a matching Bluetooth audio device is available;
4. publishing connection, battery, codec, noise-mode, and capability state on the main actor;
5. closing timers and native/Bluetooth resources on disconnect or failure;
6. implementing only the common commands advertised through `HeadphoneCapabilities`;
7. exposing `restoreOnConnectEnabled` and persisting a device-specific profile through `HeadphoneProfileStore`.
...
`HeadphoneManager`, the menu-bar device list, and Control Center use the protocol rather than vendor types. A vendor-specific settings screen may receive its concrete provider as an environment object, as the B&W and Sony screens do.
