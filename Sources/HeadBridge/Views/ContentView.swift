...
struct ContentView: View {
    ...
    var body: some View {
        NavigationSplitView {
            List(selection: $selection) {
                ...
                Section("Headphones") {
                    ForEach(manager.providers, id: \.providerID) { provider in
                        ProviderSidebarRow(provider: provider)
                            .tag(SettingsDestination.provider(provider.providerID))
                    }
                }
            }
    ...
    @ViewBuilder
    private func providerSettings(providerID: String) -> some View {
        if providerID == controller.providerID {
            BWDeviceSettingsView()
        } else if providerID == sony.providerID {
            SonyDeviceSettingsView()
        } else if let provider = manager.providers.first(where: { $0.providerID == providerID }) {
            GenericProviderSettingsView(provider: provider)
        } else {
            ContentUnavailableView("Provider unavailable", systemImage: "headphones")
        }
    }
}
...
struct HeadphoneProviderSidebarSnapshot: Equatable {
    ...
    @MainActor
    init(provider: any HeadphoneProvider) {
        name = provider.connectedName
...
final class HeadphoneProviderSidebarModel: ObservableObject {
    ...
    init(provider: any HeadphoneProvider) {
        ...
        snapshot = HeadphoneProviderSidebarSnapshot(provider: provider)
        cancellable = provider.objectWillChange
            ...
            .sink { [weak self] _ in
                ...
                let updated = HeadphoneProviderSidebarSnapshot(provider: self.provider)
...
private struct ProviderSidebarRow: View {
    ...
    init(provider: any HeadphoneProvider) {
        _model = StateObject(
            wrappedValue: HeadphoneProviderSidebarModel(provider: provider)
        )
    }
...
private enum BWDeviceSection: String, CaseIterable, Identifiable {
...
private struct BWDeviceSettingsView: View {
    @EnvironmentObject private var controller: BowersWilkinsProvider
    @EnvironmentObject private var settings: AppSettings
    @State private var section: BWDeviceSection = .controls

    var body: some View {
        VStack(spacing: 0) {
            DeviceDetailHeader(
                name: controller.connectedName,
            ...
            if settings.developerModeEnabled {
                Picker("Bowers & Wilkins section", selection: $section) {
                    ForEach(BWDeviceSection.allCases) { item in
...
private struct SonyControlsView: View {
    ...
    var body: some View {
        Form {
            ...
            if let battery = sony.batteryPercent {
                Section("Status") {
                    LabeledContent("Battery") {
                        HStack(spacing: 5) {
                            Image(systemName: HeadphoneBatterySymbol.name(for: battery))
                            if sony.isCharging == true { Image(systemName: "bolt.fill") }
                            Text("\(battery)%").monospacedDigit()
                        }
                    }
                    if let codec = sony.codecName { LabeledContent("Codec") { Text(codec) } }
                }
            }
    ...
    var body: some View {
        Form {
            ...
            Section("Status") {
                LabeledContent("Battery") {
                    HStack(spacing: 5) {
                        Image(systemName: HeadphoneBatterySymbol.name(for: controller.batteryPercent))
                        if controller.isCharging == true { Image(systemName: "bolt.fill") }
                        Text(controller.batteryPercent.map { "\($0)%" } ?? "—")
                            .monospacedDigit()
                    }
                }
                LabeledContent("Source") { Text(controller.audioSource).foregroundStyle(.secondary) }
            }
...
private struct TransportLogView: View {
    @EnvironmentObject private var controller: BowersWilkinsProvider

    var body: some View {
        List(controller.log.reversed()) { entry in
            HStack(alignment: .top, spacing: 8) {
                Text(entry.direction.rawValue)
                    .font(.caption.bold().monospaced())
                    .foregroundStyle(
                        entry.direction == .incoming ? .green : entry.direction == .outgoing ? .blue : .secondary
                    )
