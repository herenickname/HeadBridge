...
ARCHITECTURES="$(lipo -archs "$EXTRACTED_APP/Contents/MacOS/HeadBridge")"
[[ " $ARCHITECTURES " == *" arm64 "* && " $ARCHITECTURES " == *" x86_64 "* ]] \
    || fail "the release is not universal: $ARCHITECTURES"
codesign --verify --deep --strict "$EXTRACTED_APP" \
    || fail "the downloaded app has an invalid ad-hoc signature"
