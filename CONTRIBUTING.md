...
1. recognize only paired-device names belonging to its vendor/protocol family;
2. prefilter only matching Core Audio devices;
3. attach a control transport only while matching Bluetooth audio is present;
4. own no more than one live transport for its active session;
5. complete initial reads before publishing `.ready`;
6. derive capabilities from replies or a documented model profile;
7. bound parsers and queues, and ignore callbacks from stale sessions;
8. stop scans, timers, tasks, notifications, and native transports on every
   disconnect or failure path;
9. persist restore profiles by stable `providerID` and device ID;
10. expose only controls that are safe and supported by the connected device.
